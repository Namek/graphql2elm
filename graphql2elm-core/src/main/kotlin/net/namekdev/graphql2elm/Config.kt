...
data class CodeEmitterConfig(
     ...
     * @see RegisteredType
     */
    fun isKnownType(type: ANonGenericType): Boolean {
        return when (type) {
            is TScalar -> {
    ...
    fun backendTypeToFrontendDecoder(type: ANonGenericType): String {
    ...
    fun getType(type: AType): RegisteredType? {
        val frontendName = backendTypeToFrontendType(type.name)
        if (type is TScalar && isStandardScalarElmType(frontendName)) {
            return null
        }
