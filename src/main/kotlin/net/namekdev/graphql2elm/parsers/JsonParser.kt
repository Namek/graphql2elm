...
class JsonParser(private val buffer: String) : AbstractParser() {
    ...
    private fun obj(): ValueObject = block("ValueObject", {
        ...
        val pairs = mutableListOf<ObjPair>()
    ...
    class ValueObject(val pairs: List<ObjPair>) : Value() {
        val isEmpty = pairs.isEmpty()

        operator fun get(key: String): Value =
                pairs.firstOrNull { it.key.equals(key) }!!.value

        fun has(key: String): Boolean =
                pairs.any { it.key.equals(key) }
