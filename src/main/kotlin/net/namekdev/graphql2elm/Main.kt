...
fun main(args: Array<String>) {
    val typeSystemStr = queryTypeSystem()
    val typeSystem = parseTypeSystem(typeSystemStr)
...
fun parseTypeSystem(s: String): TypeSystem {
    val jsonRoot = JSONObject(s).getJSONObject("data").getJSONObject("__schema")
    ...
    for (i in 0 until jsonTypes.length()) {
        ...
        val enumValues =
            if (kind == Kind.ENUM)
                jsonType.getJSONArray("enumValues")
                    .map {
                        (it as JSONObject).getString("name")
                    }
            else null

        val type = AType(name, kind, enumValues)
    ...
    for (i in 0 until jsonTypes.length()) {
        ...
        val type = types.types[name]!!

        if (jsonType.has("fields") && !jsonType.isNull("fields")) {
            ...
            for (j in 0 until jsonFields.length()) {
                ...
                val fieldType: AType =
                    if (typeKind == Kind.LIST) {
                        ...
                        newListType.ofType = types.types[fieldSubTypeName]
                        newListType
                    }
                    else {
...
class SelectedFieldOutput(val types: TypeSystem) {
    val operations = mutableListOf<OperationDef>()
}

class QueryParser(val types: TypeSystem) : GraphQLBaseListener() {
    val output = SelectedFieldOutput(types)
    ...
    override fun enterOperationDefinition(ctx: GraphQLParser.OperationDefinitionContext) {
        ...
        val op = OperationDef(opType, ctx.NAME()?.symbol?.text)
        val selections = ctx.selectionSet()

        op.fields = traverseFields(selections, op, listOf())
        output.operations.add(op)
    }

    private fun traverseFields(selections: GraphQLParser.SelectionSetContext, op: OperationDef, path: List<String>): List<AField> {
        val selectedFields = mutableListOf<AField>()
        ...
        for (selection in selections.selection()) {
            ...
            if (fieldCtx != null) {
                ...
                val field = types.findFieldByPath(op.opType, path + fieldName)
                ...
                if (fieldCtx.selectionSet() != null) {
                    fieldType.fields.addAll(
                        traverseFields(fieldCtx.selectionSet(), op, path + fieldName)
                    )
                }
            }
        }
