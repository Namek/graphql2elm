...
fun main(args: Array<String>) {
    ...
    val elmCode = emitElmQuery(listener.output.operations[0])
    print(elmCode)
...
fun parseSchemaJson(schemaJson: String): Schema {
    ...
    for (i in 0 until jsonTypes.length()) {
        ...
        for (j in 0 until jsonFields.length()) {
            ...
            val typeKind = Kind.valueOf(jsonFieldType.getString("kind"))

            val newField: QField =
                if (typeKind == Kind.OBJECT) {
                    ...
                    val objectField = QObjectField(fieldName, fields, objectType)
                    fieldsToFillWithTypes.add(objectField)
                    objectField
                }
                else if (typeKind == Kind.SCALAR) {
                    val scalarType = schema[typeName!!] as QScalarType
                    QScalarField(fieldName, scalarType)
                }
                else if (typeKind == Kind.LIST) {
                    ...
                    val listField = QListField(fieldName, ofType, selectedFields)
                ...
                else if (typeKind == Kind.ENUM) {
                    val enumType = schema[typeName!!] as QEnumType
                    QEnumField(fieldName, enumType)
                }
                else {
...
//                val fieldType: AType =
//                    if (typeKind == Kind.LIST) {
//                        val newListType = AType(typeName, typeKind, null)
//                        val jsonFieldTypeOfType = jsonFieldType.getJSONObject("ofType")
//                        val fieldSubTypeName = jsonFieldTypeOfType.getString("name")
//
//                        newListType.ofType = schema.schema[fieldSubTypeName]
//                        newListType
//                    }
//                    else {
...
//                    }
//
//                val field = AField(fieldName, fieldType)
            ...
            if (fieldCtx != null) {
                ...
                val selectedField: QField =
                    if (field is QObjectField) {
                        ...
                        QObjectField(field.name, subFields, field.fullType)
                    }
                    else if (field is QListField) {
                        ...
                        QListField(field.name, field.ofType, selectedSubFields)
                    }
                    else {
                        field
                    }
...
//                val fieldType = field.type.copy(isFullInfo = false)
//                fieldType.fields = ArrayList<AField>()
//
//                selectedFields.add(field.copy(type = fieldType, isFullInfo = false))
//
//                if (fieldCtx.selectionSet() != null) {
//                    fieldType.fields.addAll(
//                        traverseFields(fieldCtx.selectionSet(), opType, path + fieldName)
//                    )
//                }
            }
